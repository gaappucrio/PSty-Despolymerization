...
def MC(V, Na, RT, T, t, tend,time_interval,X,R,D):
    ...
    R[23] = kf*Dsoma
    print("R initial: Dsoma / R[23]:", Dsoma, R[23])
    ...
    while t < tend:
        ...
        if   rnd <= R[0]:            # Mid-chain beta scission of lengthy polymer
            ...
            if (i) == (0):
                ...
                for j in range(1,len(D),1):
                    ...
                    if SomaD >= i: 
                        if D[int(j)] <= 0: 
                            print("R15: D[int(j)] - 1 < 0. i:", i, "j:", j, "X[0]", X[0], ". D[:5]", D[int(j)])
                        break    
            ...
            X[0]      -= 1   # Number of saturated polystyrene molecules
            D[int(j)] -= 1   # j: length, D[j] -- saturated PS molecule length j 
            Dsoma -= (j)     # number of C-C bounds
            ...
            if len(Pprimary) < i:
                print("Error R[41]. len(Pprimary); i", len(Pprimary), i)
        ...
        X[12] += D[1] + D1Unsat[1] + D2Unsat[1] # sum(D) === X[0]; sum(D1Unsat) === X[1]
        Dsoma       -= (D[1]) 
        ...
        # value2 = np.int(np.sum(X[:24])*EvaporationFactor)
        value2 = np.sum(X[:24])*EvaporationFactor
        somatemporaria = np.sum(X[:24])
        if X[19] > 0:
            # value = np.int(X[19]*EvaporationFactor)
            # value = np.int(X[19]*value2/somatemporaria)
            value = X[19]
            X[24] += value    # Toluene evaporated
            X[19] -= value    # Tolune liquid
        if X[18] > 0:
            # value = np.int(X[18]*EvaporationFactor)
            # value = np.int(X[18]*value2/somatemporaria)
            value = X[18]
            X[25] += value   # Ethylbenzene evaporated   
            X[18] -= value
        if X[12] > 0:
            # value = np.int(X[12]*EvaporationFactor)
            # value = np.int(X[12]*value2/somatemporaria)
            value = X[12]
            # print(X[12]/np.sum(X[:24]), value2, value, np.int(X[12]*value2/np.sum(X[:24])))
            ...
            if X[12] < 0:
                ...
                break             
        if X[13] > 0:
            # value = np.int(X[13]*EvaporationFactor)
            # value = np.int(X[13]*value2/somatemporaria)
            value = X[13]
            X[27] += value    # aMS evaporated
            X[13] -= value
        if X[14] > 0:
            # value = np.int(X[14]*EvaporationFactor)
            # value = np.int(X[14]*value2/somatemporaria)
            value = X[14]
            X[28] += value    # Dimer evaporated
            X[14] -= value
        if X[15] > 0:
            # value = np.int(X[15]*EvaporationFactor)
            # value = np.int(X[15]*value2/somatemporaria)
            value = X[15]
            X[29] += value    # DPP evaporated
            X[15] -= value
        if X[16] > 0:
            # value = np.int(X[16]*EvaporationFactor)
            # value = np.int(X[16]*value2/somatemporaria)
            value = X[16]
